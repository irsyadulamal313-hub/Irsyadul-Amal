...
import { X, Heart, Calendar, Clock, MapPin, Image as ImageIcon } from 'lucide-react';
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col border border-[#CCFBF1]">
        ...
        <div className="p-4 sm:p-5 border-b border-gray-100 flex items-center justify-between bg-[#F0FAFA]">
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 bg-[#008284] text-white text-[11px] font-semibold rounded-full">
              {program.category}
            </span>
            <span className="text-xs font-medium text-teal-800 bg-teal-100 px-2.5 py-0.5 rounded-full">
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-5 text-xs sm:text-sm text-gray-700">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 p-3.5 bg-[#F0FAFA] rounded-2xl border border-[#CCFBF1]">
            <div>
              <span className="text-[10px] uppercase font-medium text-gray-400">Target Dana</span>
              <p className="font-semibold text-[#071F20] text-xs sm:text-sm mt-0.5">
                {program.targetAmountText || 'Target akan diperbarui'}
              </p>
            </div>
            <div>
              <span className="text-[10px] uppercase font-medium text-gray-400">Dana Terkumpul</span>
              <p className="font-semibold text-[#008284] text-xs sm:text-sm mt-0.5">
                {program.collectedAmountText || 'Data donasi akan diperbarui'}
              </p>
            </div>
            <div>
              <span className="text-[10px] uppercase font-medium text-gray-400">Penerima Manfaat</span>
              <p className="font-semibold text-[#071F20] text-xs sm:text-sm mt-0.5">
                {program.targetRecipients || 'Data akan diperbarui'}
              </p>
            </div>
            <div>
              <span className="text-[10px] uppercase font-medium text-gray-400">Jumlah Donatur</span>
              <p className="font-semibold text-[#071F20] text-xs sm:text-sm mt-0.5">
          ...
          <div className="space-y-3">
            ...
            {program.background && (
              <div className="bg-gray-50 rounded-xl p-3 border-l-4 border-[#008284] space-y-1">
                <span className="font-medium text-xs text-gray-900">Latar Belakang:</span>
                <p className="text-xs text-gray-600 leading-relaxed font-normal">{program.background}</p>
              </div>
            )}
            {program.objective && (
              <div className="bg-gray-50 rounded-xl p-3 border-l-4 border-amber-400 space-y-1">
                <span className="font-medium text-xs text-gray-900">Tujuan Program:</span>
                <p className="text-xs text-gray-600 leading-relaxed font-normal">{program.objective}</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-[#E6F7F7] border border-[#CCFBF1] rounded-2xl p-4 space-y-1.5">
            <div className="flex items-center gap-1.5 text-xs font-semibold text-[#006769]">
              <Clock className="w-3.5 h-3.5" />
              Update Terbaru Penyaluran / Pengerjaan
            </div>
            <p className="text-xs text-[#004D4F] leading-relaxed font-normal">
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between gap-3">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-xl bg-white border border-gray-200 hover:bg-gray-100 text-gray-700 text-xs font-medium cursor-pointer"
          >
            Tutup
          </button>
          ...
          <button
            ...
            className="px-6 py-2.5 rounded-xl bg-[#008284] hover:bg-[#006769] text-white text-xs sm:text-sm font-semibold shadow-xs flex items-center gap-2 cursor-pointer active:scale-98"
          >
